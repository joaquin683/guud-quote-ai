...
const ESTADOS = {
  ...
  agendado:    { label: 'Reunion agendada', color: '#60a5fa', bg: 'rgba(96,165,250,0.12)' },
              ...
              <div key={p.id} style={{ background: '#111', border: '1px solid #1f1f1f', borderRadius: 10, padding: '16px 20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, flexWrap: 'wrap' }}>
                  ...
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0 }}>
                    ...
                    {(() => { const e = ESTADOS[p.estado] || { color: '#888', bg: 'rgba(136,136,136,0.1)' }; return (<select value={p.estado || ''} onChange={ev => actualizarEstado(p.id, ev.target.value)} onClick={ev => ev.stopPropagation()} style={{ fontSize: 11, fontWeight: 600, padding: '3px 10px', borderRadius: 20, background: e.bg, color: e.color, border: '1px solid ' + e.color + '55', fontFamily: 'inherit', cursor: 'pointer', outline: 'none', appearance: 'none', WebkitAppearance: 'none', MozAppearance: 'none' }}>{!ESTADOS[p.estado] && p.estado ? <option value={p.estado} style={{ background: '#111', color: '#F2F0E8' }}>{p.estado}</option> : null}{Object.entries(ESTADOS).map(([k, v]) => <option key={k} value={k} style={{ background: '#111', color: '#F2F0E8' }}>{v.label}</option>)}</select>); })()}
