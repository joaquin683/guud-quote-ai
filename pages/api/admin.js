...
export default async function handler(req, res) {
  ...
  try {
    ...
    let query = supabase.from('proyectos').select('*').order('creado_en', { ascending: false }).limit(parseInt(limit))
    ...
    const agendados = data.filter(p => p.reunion_agendada === true).length
    const prices = data.filter(p => p.precio_estimado_min)
    const avgPrecio = prices.length ? Math.round(prices.reduce((a, p) => a + p.precio_estimado_min, 0) / prices.length) : 0
    res.status(200).json({ proyectos: data || [], metrics: { total, cotizados, agendados, avgPrecio } })
  } catch(e) {
